...
class VCDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        t_size, dim_size = acoustic_token.shape

        semantic_token = semantic_token + 1  # padding
        acoustic_token = acoustic_token + 1 + self.hparams.num_semantic  # padding + len(text)
    ...
    def collate_fn(self, batches):
        ...
        max_seq_len = max(first_seq.shape[0] for first_seq, _, _, _,_, in batches)
        for first_seq, full_seq, pos_id, seq_tag, utt in batches:
